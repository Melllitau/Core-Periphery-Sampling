...
mkdir -p topicGPT/data/output/wiki logs

LOG=logs/wiki.log
echo "[$(date)] Lancando pipeline Wiki..." | tee -a "$LOG"
...
for PHASE in generation refinement assignment; do
    ...
    while true; do
        python3 -u topicGPT/run_wiki.py --phase "$PHASE" \
            >> "$LOG" 2>&1 &
        PID=$!
        echo "  PID=$PID"
...
echo "============================================================"
echo "[$(date)] PIPELINE WIKI COMPLETO!"
echo "============================================================"
