...
def merge_topics(
    ...
    tree_ckpt_file = os.path.join(checkpoint_dir, "refine_tree_ckpt.md") if checkpoint_dir else None
    if ckpt_file and os.path.exists(ckpt_file):
        with open(ckpt_file, "r") as f:
            ckpt = json.load(f)
        all_pairs = ckpt["all_pairs"]
        orig_new = ckpt["mapping"]
        if tree_ckpt_file and os.path.exists(tree_ckpt_file):
            topics_root = TopicTree().from_topic_list(tree_ckpt_file, from_file=True)
            print(f"  [Checkpoint] Árvore restaurada de {tree_ckpt_file}")
        else:
    ...
    while len(new_pairs) > 1:
        ...
        if ckpt_file and iteration % 10 == 0:
            with open(ckpt_file, "w") as f:
                json.dump({"all_pairs": all_pairs, "mapping": orig_new}, f)
            if tree_ckpt_file:
                topics_root.to_file(tree_ckpt_file)
