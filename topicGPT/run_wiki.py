"""
Runner do pipeline TopicGPT para o dataset Wiki.

Uso:
    python topicGPT/run_wiki.py --phase generation
    python topicGPT/run_wiki.py --phase refinement
    python topicGPT/run_wiki.py --phase assignment
...
DATASETS_DIR = "EDA-ESG-Reports/Sampling-Method/data/wiki"
OUTPUT_DIR = "topicGPT/data/output/wiki"
...
ALL_ROUNDS = [
    {
        "name": "round_0_cps",
        "gen_data": f"{DATASETS_DIR}/generation_wiki_CPS.jsonl",
        "assign_data": f"{DATASETS_DIR}/assignment_wiki_CPS.jsonl",
    },
    {
        "name": "round_1_random",
        "gen_data": f"{DATASETS_DIR}/generation_wiki_random1.jsonl",
        "assign_data": f"{DATASETS_DIR}/assignment_wiki_random1.jsonl",
    },
    {
        "name": "round_2_random",
        "gen_data": f"{DATASETS_DIR}/generation_wiki_random2.jsonl",
        "assign_data": f"{DATASETS_DIR}/assignment_wiki_random2.jsonl",
    },
    {
        "name": "round_3_random",
        "gen_data": f"{DATASETS_DIR}/generation_wiki_random3.jsonl",
        "assign_data": f"{DATASETS_DIR}/assignment_wiki_random3.jsonl",
    },
]
...
def run_generation(rounds):
    ...
    gen_prompt = f"{PROMPT_DIR}/generation_1_wiki.txt"
    seed_file = f"{PROMPT_DIR}/seed_1_wiki.md"
...
def run_refinement(rounds):
    ...
    refine_prompt = f"{PROMPT_DIR}/refinement_wiki.txt"
...
def run_assignment(rounds):
    ...
    assign_prompt = f"{PROMPT_DIR}/assignment_wiki.txt"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Pipeline TopicGPT — Wiki")
    ...
    for phase in phases:
        print("=" * 60)
        print(f"  WIKI — {phase.upper()} — {len(ALL_ROUNDS)} rounds")
        print("=" * 60)
